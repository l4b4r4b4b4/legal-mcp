# Legal-MCP Documentation

This directory contains extended documentation for the Legal-MCP project.
...
uv run legal-mcp --transport sse --port 8000 --host 0.0.0.0
```
...
| `legal-mcp` | fastmcp-base | This template application |
| `legal-mcp:dev` | python:3.12-slim | Development with hot reload |
...
# Build app image
docker build -f docker/Dockerfile -t legal-mcp:latest .

# Build dev image
docker build -f docker/Dockerfile.dev -t legal-mcp:dev .
```
...
```bash
# Production mode
docker run -p 8000:8000 legal-mcp:latest
...
docker run -p 8000:8000 -v $(pwd)/app:/app/app:ro legal-mcp:dev
```
...
```dockerfile
FROM ghcr.io/l4b4r4b4b4/legal-mcp-base:latest
...
# Images published to:
#   ghcr.io/l4b4r4b4b4/legal-mcp-base:latest
#   ghcr.io/l4b4r4b4b4/legal-mcp:latest
```
